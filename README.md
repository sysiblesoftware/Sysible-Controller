...
| Area | What it covers |
|---|---|
| **Dashboard / Fleet health** | The home screen shows live fleet counts (hosts, online/offline, environments) and an at-a-glance **fleet-health overview**: an OK/Warning/Critical donut plus a card **per environment** — each summarizing the worst verdict, per-verdict host counts, peak disk/memory, and aggregated problem signals (crashed/failed services, out-of-memory kills, degraded systemd), sorted worst-first and **expandable to its individual hosts** (disk/memory meters, load, the named crashed services, etc.). Gathered on demand (with optional auto-refresh), read-only, no operator sudo needed, and shown to both sysadmins and superusers (superusers also get the activity feed). |
| **Performance graphs** | A dedicated **Performance** view with environment-first time-series charts (Cockpit-style, no chart-library dependency): CPU load, memory, and disk over a selectable window (1h / 6h / 24h, with optional auto-refresh). The overview draws **one line per environment** (each a distinct color, averaged across its hosts); **click an environment to drill into one line per host** (each a distinct color). The series are fed by the **agent itself**, which samples load/memory/disk and reports them on its heartbeat (roughly once a minute) — so this needs an up-to-date agent on the host; SSH-only hosts aren't sampled. |
...
| **File System Management** | List directory contents, **view a file's contents** read-only (**View File**, output capped so a huge/binary file can't flood the pane), create/remove directories, copy/move/rename files, manage ownership, permissions, ACLs, and links, mount/unmount/resize/repair filesystems, mount **NFS and CIFS/SMB network shares** (optionally persisted to `/etc/fstab`, with CIFS credentials kept in a root-only file), configure `/etc/fstab` and quotas, and archive/compress files — organized into focused tabs (Directories & Files, Permissions, Mount/Unmount, Network Mounts, Resize & Repair, Disk Usage, fstab & Quotas, Archive). Deleting or unmounting a **system-critical file or mount** (`/`, `/etc/fstab`, `/boot`, …) is guarded: blocked for sysadmins, and a superuser is warned and must confirm. |
...
The terminal's *Send sudo password* button on Sysible Connect is itself **opt-in**: it's **off by default for every account** and a **superuser grants it per administrator** in **Settings → Administrators** (the web console enforces this server-side). This lets a superuser withhold interactive sudo-password injection from accounts that shouldn't have it, without affecting their other tools.
...
- **For the web console:** Node.js 18+ to build the front end (provisioned by the installer when available; build-time only — once `webgui/frontend/dist/` exists no Node is needed at run time), and any modern browser on a machine that can reach the controller. No desktop environment is needed on the controller itself.
...
```bash
sudo sysible_controller update
```

This pulls your recorded source checkout (`git pull --ff-only`), re-syncs it into `/opt/sysible` while **preserving your database, admin API key, TLS certificates, host records, SSH keys, and sessions**, refreshes dependencies, rebuilds the web-console front end, and restarts the services. (Re-running `sudo ./install_sysible.sh` from the checkout does the same thing.) Managed hosts run their **own** copy of the agent, so to pick up agent-side changes you redeploy the agent to each host (re-run its installer) — that's separate from updating the controller.
