...
| Area | What it covers |
|---|---|
| **Dashboard / Fleet health** | The home screen shows live fleet counts (hosts, online/offline, environments) and an at-a-glance **fleet-health overview**: an OK/Warning/Critical donut plus a per-host card for every reachable host with **disk and memory meters, load, crashed/failed services (by name), out-of-memory kills, and a degraded-systemd flag**, sorted worst-first. Gathered on demand (with optional auto-refresh) by the agent itself — read-only, no operator sudo needed — and shown to both sysadmins and superusers (superusers also get the activity feed). |
