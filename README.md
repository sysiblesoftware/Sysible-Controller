...
| **Security Administration** | Configure and troubleshoot SELinux, harden SSH access and rotate keys, review audit logs and failed logins, install security updates, set OS-level password policy, apply system hardening, run vulnerability scans, and apply system hardening. |
| **Directory Services (AD / LDAP)** | Join hosts to **Active Directory** (realmd/SSSD/adcli — installs the tooling, joins the domain, with the join password kept off the command line), leave a domain, show realm/Kerberos status, permit AD users/groups to log in, enable home-dir creation, test **LDAPS** connectivity, and write a generic SSSD LDAP/LDAPS client config. |
