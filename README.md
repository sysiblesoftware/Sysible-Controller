...
> Sysible Connect: the host list stays a plain list; the actions are grouped on the right — Fleet Actions (run a script or reboot/power-off/restart the agent across the fleet), file transfer for the selected host, SSH-enroll a new host, and RDP to a Windows host. Check hosts and click **Open Terminal** (or double-click one) to pop out a shell per host.
...
There is **no cap** on the number of administrators of any role, or on the number of managed hosts — the Community edition is uncapped. (Edition metadata lives in one place, `backend/edition.py`.)
...
This is the **Community edition**, and it is **uncapped**: there is no limit on the number of managed hosts, nor on the number of administrators of any role (Superuser, Sysadmin, or Auditor). The web console simply shows a small **Community Edition** badge in the sidebar — there's no usage-vs-limit count to watch. Edition metadata lives in one place (`backend/edition.py`).
