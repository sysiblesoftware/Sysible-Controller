...
def _resolve_admin_username(request: Request):
    """The acting operator's Controller username, or None. This is the "run as"
    identity: terminals and remote exec run as this account on the host (with its
    own sudo) instead of the SSH login user, and the audit trail attributes them to
    it. None means "no run-as" — the default account, unattributed.

    Two authenticated sources:

      * X-Sysible-Admin-Token — a Controller console login, resolved against the
        admin token store. Used by standalone Connect / an API-key attach.
      * X-Sysible-User asserted by the SLOP gateway, honored ONLY when the request
        also carries the gateway shared secret (_gateway_secret_ok: constant-time,
        and fails closed when the secret is unset).

    The second one is not optional under SLOP. An SSO operator has NO local
    Controller password — the SLOP sign-in IS the login — so there is no admin token
    to mint, and reading the token alone made this return None for every request
    that came in through the gateway. Every SSO terminal then ran as the Controller's
    default account with no attribution, which is what Connect reported as
    "API-key connect (no run-as)".

    The asserted username is resolved against the administrators table (SLOP
    provisions a row on first sign-in — see /admin/sso-provision), so a run-as can
    only ever be a real Controller administrator and never an arbitrary string off a
    header: the value reaches a host command (_become_user_command / the agent's
    pty_open), and although that path shlex-quotes it, "the header said so" is not a
    basis for choosing whose account a shell runs as.
    """
    token = request.headers.get("X-Sysible-Admin-Token")
    if token:
        from backend.db import resolve_admin_token
    ...
    # Lazy import: backend.app imports this module, and _gateway_secret_ok is the
    # single place the shared secret is compared — duplicating that check here
    # would be a second, driftable copy of a security boundary.
    from backend.app import _gateway_secret_ok
    who = (request.headers.get("x-sysible-user") or "").strip()
    if not who or not _gateway_secret_ok(request):
        return None
    # Provision on first sight, and keep the role in lockstep after that — the same
    # thing the console's BFF does on its first page load, on the same terms
    # (/admin/sso-provision's logic, reused). Doing it here matters because an
    # operator who goes straight to Sysible Connect never loads the console page, so
    # their administrator row would not exist and their shells would silently run as
    # the default account: a run-as that depended on which app you opened first.
    #
    # ValueError means either a malformed asserted name (refused before any row is
    # created) or a name already owned by a LOCALLY-managed admin. The second is an
    # identity collision we cannot resolve — SSO manages only the accounts it
    # created, and the console's SSO path refuses the same case — so fall back to no
    # run-as (the default account) rather than borrowing someone else's account.
    from backend.app import ensure_sso_account, sso_role_for
    from backend.db import get_administrator
    try:
        ensure_sso_account(who, sso_role_for(request.headers.get("x-sysible-role") or ""),
                           "gateway(run-as)")
    except ValueError:
        return None
    rec = get_administrator(who)
...
def _reject_auditor(request: Request):
    ...
    Covers both identity sources. An SSO auditor arrives with a gateway-asserted
    role and no admin token, so checking the token alone would have let them
    straight through this gate once the gateway path started resolving an identity
    at all. Both the asserted role AND the provisioned row are checked, and either
    one saying 'auditor' refuses."""
    token = request.headers.get("X-Sysible-Admin-Token")
    if token:
        from backend.db import resolve_admin_token
        admin = resolve_admin_token(token)
    ...
    if not _gateway_secret_ok(request):
        return                       # no trusted identity to judge
    if (request.headers.get("x-sysible-role") or "").strip().lower() == "auditor":
        raise HTTPException(status_code=403, detail="Auditor accounts are read-only.")
    who = (request.headers.get("x-sysible-user") or "").strip()
    if who:
        from backend.db import get_administrator
        rec = get_administrator(who)
        if rec and (rec.get("role") or "") == "auditor":
            raise HTTPException(status_code=403, detail="Auditor accounts are read-only.")
