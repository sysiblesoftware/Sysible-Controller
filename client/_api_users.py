...
def queue_op_on_hosts(host_ids, op: str, description: str = None,
    ...
    """Queue a vetted privileged OPERATION instead of a shell command, e.g.
    queue_op_on_hosts(ids, 'service.restart', unit='nginx.service'). The agent
    runs it through sysible-priv (argv-only, validated), not bash -c. Migrating a
    command-builder is just swapping its queue_command_on_hosts call for this."""
