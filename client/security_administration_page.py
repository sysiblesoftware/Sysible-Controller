...
class SecurityAdministrationPage(QWidget):
    ...
    def __init__(self):
        ...
        action_tabs.addTab(self._build_directory_tab(), "Directory (AD / LDAP)")
    ...
    def _build_directory_tab(self):
        ...
        box, g = self._group("Active Directory")
        srow = QHBoxLayout()
        btn_status = QPushButton("Realm / Domain Status")
        btn_status.clicked.connect(lambda: self._run_security_command(api.cmd_realm_status(), "Realm Status"))
        srow.addWidget(btn_status)
        btn_mkhome = QPushButton("Enable Home-Dir Creation")
        btn_mkhome.clicked.connect(lambda: self._run_security_command(api.cmd_enable_mkhomedir(), "Enable mkhomedir"))
        srow.addWidget(btn_mkhome)
        srow.addStretch()
        g.addLayout(srow)

        jrow1 = QHBoxLayout()
        jrow1.addWidget(QLabel("Domain:"))
        self.ad_domain = QLineEdit()
        self.ad_domain.setPlaceholderText("e.g. corp.example.com")
        jrow1.addWidget(self.ad_domain, 1)
        jrow1.addWidget(QLabel("Join account:"))
        self.ad_user = QLineEdit()
        self.ad_user.setPlaceholderText("e.g. Administrator")
        jrow1.addWidget(self.ad_user, 1)
        g.addLayout(jrow1)
        jrow2 = QHBoxLayout()
        jrow2.addWidget(QLabel("Password:"))
        self.ad_pass = QLineEdit()
        self.ad_pass.setEchoMode(QLineEdit.Password)
        jrow2.addWidget(self.ad_pass, 1)
        jrow2.addWidget(QLabel("Computer OU:"))
        self.ad_ou = QLineEdit()
        self.ad_ou.setPlaceholderText("optional, e.g. OU=Servers,DC=corp,DC=example,DC=com")
        jrow2.addWidget(self.ad_ou, 1)
        btn_join = QPushButton("Join Domain")
        btn_join.setStyleSheet("font-weight: bold;")
        btn_join.clicked.connect(self.run_join_ad)
        jrow2.addWidget(btn_join)
        g.addLayout(jrow2)
        g.addWidget(self._dir_hint(
        ...
        prow = QHBoxLayout()
        prow.addWidget(QLabel("Permit user/group:"))
        self.ad_permit = QLineEdit()
        self.ad_permit.setPlaceholderText("e.g. jdoe@corp.example.com  or a group name")
        prow.addWidget(self.ad_permit, 1)
        self.ad_permit_group = QCheckBox("Is a group")
        prow.addWidget(self.ad_permit_group)
        btn_permit = QPushButton("Permit Login")
        btn_permit.clicked.connect(self.run_permit)
        prow.addWidget(btn_permit)
        prow.addWidget(QLabel("Leave:"))
        self.ad_leave_domain = QLineEdit()
        self.ad_leave_domain.setPlaceholderText("domain")
        prow.addWidget(self.ad_leave_domain, 1)
        btn_leave = QPushButton("Leave Domain")
        btn_leave.clicked.connect(self.run_leave_ad)
        prow.addWidget(btn_leave)
        g.addLayout(prow)
        layout.addWidget(box)

        box2, g2 = self._group("LDAP / LDAPS")
        lrow = QHBoxLayout()
        lrow.addWidget(QLabel("Test server:"))
        self.ldaps_server = QLineEdit()
        self.ldaps_server.setPlaceholderText("e.g. ldap.example.com")
        lrow.addWidget(self.ldaps_server, 1)
        lrow.addWidget(QLabel("Port:"))
        self.ldaps_port = QLineEdit("636")
        self.ldaps_port.setMaximumWidth(70)
        lrow.addWidget(self.ldaps_port)
        lrow.addWidget(QLabel("Base DN:"))
        self.ldaps_base = QLineEdit()
        self.ldaps_base.setPlaceholderText("optional, e.g. dc=example,dc=com")
        lrow.addWidget(self.ldaps_base, 1)
        btn_test = QPushButton("Test LDAPS")
        btn_test.clicked.connect(self.run_test_ldaps)
        lrow.addWidget(btn_test)
        g2.addLayout(lrow)
        crow = QHBoxLayout()
        crow.addWidget(QLabel("Client server:"))
        self.ldap_server = QLineEdit()
        self.ldap_server.setPlaceholderText("e.g. ldap.example.com")
        crow.addWidget(self.ldap_server, 1)
        crow.addWidget(QLabel("Base DN:"))
        self.ldap_base = QLineEdit()
        self.ldap_base.setPlaceholderText("e.g. dc=example,dc=com")
        crow.addWidget(self.ldap_base, 1)
        crow.addWidget(QLabel("Protocol:"))
        self.ldap_scheme = QComboBox()
        self.ldap_scheme.addItems(["ldaps", "ldap+starttls"])
        crow.addWidget(self.ldap_scheme)
        btn_cfg = QPushButton("Configure SSSD")
        btn_cfg.clicked.connect(self.run_configure_ldap)
        crow.addWidget(btn_cfg)
        g2.addLayout(crow)
        g2.addWidget(self._dir_hint(
            "Test checks the TLS handshake/cert (and, with a base DN, runs an anonymous search). "
            "Configure SSSD writes a basic /etc/sssd/sssd.conf for a generic (non-AD) LDAP directory "
            "and restarts SSSD. For Active Directory, use Join Domain above."))
        layout.addWidget(box2)

        layout.addStretch()
        return panel

    def _dir_run(self, label, factory):
        try:
            cmd = factory()
        except ValueError as e:
            QMessageBox.warning(self, "Invalid input", str(e))
            return
        self._run_security_command(cmd, label)

    def run_join_ad(self):
        self._dir_run("Join Active Directory", lambda: api.cmd_join_ad(
            self.ad_domain.text(), self.ad_user.text(), self.ad_pass.text(), self.ad_ou.text()))

    def run_permit(self):
        self._dir_run("Permit Login", lambda: api.cmd_realm_permit(
            self.ad_permit.text(), self.ad_permit_group.isChecked()))

    def run_leave_ad(self):
        self._dir_run("Leave Domain", lambda: api.cmd_leave_ad(self.ad_leave_domain.text()))

    def run_test_ldaps(self):
        self._dir_run("Test LDAPS", lambda: api.cmd_test_ldaps(
    ...
    def run_configure_ldap(self):
        ...
        self._dir_run("Configure LDAP Client", lambda: api.cmd_configure_ldap_client(
