...
#
# The .desktop file can't call pkexec directly though - a .desktop
# Exec= line gets no shell expansion, so a literal "$DISPLAY" in
# it is passed through as that literal string, not substituted, and
# pkexec on its own only auto-forwards DISPLAY/XAUTHORITY (the X11
# pair), never WAYLAND_DISPLAY/XDG_RUNTIME_DIR. The desktop's icon
# would then pop the auth prompt, accept the password, and the
# elevated GUI process would have nowhere to actually display itself
# - exactly the "auth window appears but the GUI never does" failure.
# So Exec= instead points at a tiny wrapper script that runs first,
# still as the logged-in user (this is the only point in the chain
# where these variables are guaranteed to reflect the real graphical
# session), and only then calls pkexec with them passed through
# explicitly as literal values.
# =========================================================
...
if command -v pkexec >/dev/null 2>&1 && [[ -d "$DESKTOP_DIR" ]]; then
  ...
  LAUNCH_SCRIPT="$BASE/launch_gui.sh"

  cat > "$LAUNCH_SCRIPT" <<'EOF'
#!/bin/bash
# Generated by install_sysible.sh - do not edit by hand, it gets
# overwritten on every (re)install. Runs unprivileged, as whoever
# clicked the application menu icon; see install_sysible.sh's
# "INSTALL THE APPLICATION MENU LAUNCHER" section for why this exists
# instead of pkexec being called directly from the .desktop file.
exec pkexec env \
  DISPLAY="$DISPLAY" \
  ...
  /usr/local/bin/sysible_controller gui
EOF
